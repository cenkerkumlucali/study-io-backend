            ...
            Task.Run(async () =>
            {
                try
                {
                    MailMessageData? mailMessageData =
                        JsonSerializer.Deserialize<MailMessageData>(Encoding.UTF8.GetString(ea.Body.Span));
                    // var task =
                    switch (mailMessageData.RoutingKey)
                    {
                        case 0:
                            await _mailSender.SendPasswordResetMailAsync(mailMessageData.Email,
                                mailMessageData.FirstName + " " + mailMessageData.LastName,
                                mailMessageData.ActivationKey);
                            break;
                        case 1:
                            await _mailSender.SendEnableEmailAuthenticatorAsync(mailMessageData.Email,
                                mailMessageData.FirstName, mailMessageData.LastName,
                                mailMessageData.VerifyEmailUrlPrefix, mailMessageData.ActivationKey);
                            break;
                        case 2:
                            await _mailSender.SendAuthenticatorCodeAsync(mailMessageData.Email,
                                mailMessageData.FirstName,
